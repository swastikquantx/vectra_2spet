...
if (match) {
  ...
  const p1 = solHtml.indexOf('g("nav",{className:"hidden lg:flex items-center gap-5 xl:gap-6 text-[13.5px] font-medium"');
  const p2 = solHtml.indexOf('children:"Create Studio / Login"})', p1) + 'children:"Create Studio / Login"})'.length;
  
  if (p1 !== -1 && p2 > p1) {
    const oldNav = solHtml.substring(p1, p2);
    ...
    console.log('Solutions old nav bounds not found');
  }

  // Same for Auth
  let authHtml = Buffer.from(pages['Auth-Genz-Registration.html'], 'base64').toString('utf-8');
  const ap1 = authHtml.indexOf('m("nav",{className:"hidden xl:flex items-center gap-7 text-[13.5px] font-medium text-[#4B5563]"');
  const ap2 = authHtml.indexOf('children:"Create Studio / Login"})', ap1) + 'children:"Create Studio / Login"})'.length;
  ...
  if (ap1 !== -1 && ap2 > ap1) {
    ...
    console.log('Auth old nav bounds not found');
  }
